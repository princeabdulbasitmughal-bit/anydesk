...
- [x] Upgrade the direct NanoID runtime resolution to the audit-fixed 5.1.16 line and verify whether the managed Vite runtime can share that resolution without an unsupported plugin upgrade. Both the direct dependency and managed Vite runtime resolve to 5.1.16; the production audit contains no NanoID entry, and frozen lockfile validation, 62 tests, TypeScript checks, and a 1280 MiB-capped production build pass.
- [x] Upgrade Recharts to the React 19-compatible 3.10.1 line, validate TrackLab’s bar/scatter chart API use against the documented migration changes, and verify that the residual Lodash audit path is removed without fabricating chart data. TrackLab uses no documented breaking internal chart-state APIs; shared tooltip/legend helpers now use Recharts 3 payload contracts and metric formatting handles optional non-numeric values without inventing zeroes. The production audit exits cleanly with no Lodash entry, and frozen lockfile validation, 63 tests, TypeScript checks, and a 1280 MiB-capped production build pass.
- [x] Reconcile the readiness audit’s temporary upstream advisory boundary with the final clean production audit result, preserving the distinction between dependency posture and external-provider activation requirements. The audit now records the final fixed baseline and clean production-audit exit while retaining the separate owner-authorized provider and real-data boundaries.
- [x] Normalize hosted-job failure details through the existing secret-safe diagnostic redactor before database persistence, researcher responses, and terminal-email propagation; add regression coverage for token and authorization-header redaction. Hosted submission, remote terminal-status, and administrator-recorded failure details now use one redacted path before persistence or notification; the full 64-test suite verifies an authorization-header token is redacted in stored, returned, and email-propagated details, while the frozen lockfile, clean production audit, TypeScript, and capped production build pass.
