...
describe("Reproducibility Ledger", () => {
  it("marks absent evidence as missing without fabricating research records", () => {
    const ledger = buildReproducibilityLedger({ experiment, datasets: [], configurations: [], runs: [], metricsByRun: {}, tracksByRun: {}, reports: [] });
  ...
  it("preserves only supplied evidence links and never substitutes metric values", () => {
    const ledger = buildReproducibilityLedger({
      ...
      runs: [{ id: 12, datasetId: 8, modelConfigurationId: 9, runType: "inference", status: "completed" }],
    ...
    expect(ledger.manifest.evidence.runs[0]).toMatchObject({ id: 12, datasetId: 8, modelConfigurationId: 9, metricRecord: { accuracy: null, efficiency: null, fakeRate: null }, reconstructedPointCount: 0 });
