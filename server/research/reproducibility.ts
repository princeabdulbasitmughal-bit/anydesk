...
export function buildReproducibilityLedger(input: ReproducibilityLedgerInput) {
  const { datasets, configurations, runs, metricsByRun, tracksByRun, finding, reports } = input;
  ...
  const checks = [
    ...
    check("run-links", "Run lineage", !runs.length ? "missing" : runsMissingDataset.length || runsMissingConfiguration.length ? "attention" : "present", !runs.length ? "No run has been recorded yet." : runsMissingDataset.length || runsMissingConfiguration.length ? "One or more runs cannot be linked to a currently available dataset or configuration record." : `${runs.length} run${runs.length === 1 ? "" : "s"} retain dataset and configuration links.`),
  ...
  const hasActualEvidence = Boolean(datasets.length || configurations.length || runs.length || finding || reports.length);
  const manifest = {
    schemaVersion: "tracklab-reproducibility-ledger/v1",
