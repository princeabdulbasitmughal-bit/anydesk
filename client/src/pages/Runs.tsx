...
export default function Runs() {
  ...
  const prerequisites = getRunPrerequisites(Boolean(datasets.data?.length), Boolean(configs.data?.length));
  ...
  const canReview = canStart && Boolean(selectedExperimentId && selectedDataset && selectedConfiguration);
  ...
  const confirmRun = () => {
    if (!selectedExperimentId || !selectedDataset || !selectedConfiguration || !acknowledged) return;
    trigger.mutate({ experimentId: selectedExperimentId, datasetId: selectedDataset.id, modelConfigurationId: selectedConfiguration.id, runType });
  };
  ...
  return <>
    ...
    <ExperimentGate>
      <div className="grid gap-5 xl:grid-cols-[0.9fr_1.4fr]">
        <form onSubmit={openReview} className="rounded-2xl border border-cyan-300/15 bg-gradient-to-b from-cyan-300/[0.06] to-slate-900/40 p-6">
          ...
          {canStart && !canReview ? <p className="mt-3 text-xs leading-5 text-slate-500">Select both a dataset and a versioned configuration to review the run.</p> : null}
        </form>
    ...
    <Dialog open={reviewOpen} onOpenChange={closeReview}>
      <DialogContent className="border-cyan-300/20 bg-slate-950 text-slate-100 sm:max-w-xl">
        ...
        <div className="flex items-start gap-3 rounded-xl border border-white/10 bg-slate-900/40 p-4"><Checkbox id="run-launch-acknowledgement" checked={acknowledged} onCheckedChange={value => setAcknowledged(value === true)} className="mt-0.5 border-slate-500 data-[state=checked]:bg-cyan-300 data-[state=checked]:text-slate-950" /><Label htmlFor="run-launch-acknowledgement" className="cursor-pointer text-sm leading-6 text-slate-200">I confirm that this submission uses the selected dataset and versioned configuration, and I understand that authorized remote execution may consume provider resources.</Label></div>
