...
import { buildProtocolReadiness } from "../../../server/research/protocolReadiness";
...
export default function Protocols() {
  ...
  const readiness = useMemo(() => buildProtocolReadiness(latest.data), [latest.data]);
  ...
  return <><PageHeading eyebrow="Pre-execution integrity" title="Experiment Protocol" description="Record the real research objective, detector context, evaluation plan, and review criteria before execution. Every save creates a new version; this workflow never supplies claims or expected scientific outcomes.">
    <Badge className="border border-cyan-200/20 bg-cyan-300/10 px-3 py-1.5 text-xs font-semibold text-cyan-100">{latest.data ? `${readiness.filter(item => item.status === "documented").length}/4 sections documented · v${latest.data.version}` : "No protocol recorded"}</Badge>
  </PageHeading>
